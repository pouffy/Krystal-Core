...
public abstract class AbstractArrowMixin extends Projectile {
    ...
    private float attributedKnockback(float original) {
        var owner = this.getOwner();
        if (owner instanceof LivingEntity living) {
            AttributesHelper helper = AttributesHelper.create(living);
            return helper.modifyProjectileKnockback(original);
        }
        return original;
    }
}
